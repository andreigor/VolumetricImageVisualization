...
int main(int argc, char *argv[]){
    ...
    if ((argc != 5)){
        printf("usage extract_features: <P1> <P2> <P3> <P4>\n");
        printf("P1: input grayscale 3D image\n");
        printf("P2: kernel bank (.mmk)\n");
    ...
    int adj_radius           = atoi(argv[3]);

    
    iftMImage *mimg = iftImageToMImage(img, LAB_CSPACE);
    iftMImage *aux  = iftMAddFrame(mimg, adj_radius, adj_radius, adj_radius, 0);
    iftDestroyMImage(&mimg);
    mimg = aux;
    
    iftMImage *activ         = iftMMLinearFilter(mimg, kernel_bank);

    // printf("Tamanho antes de remover: %d %d %d\n", activ->xsize, activ->ysize, activ->zsize);

    // aux = iftMRemFrame(activ, (3 - 1)/2, (3 - 1)/2, 0);
    // printf("Tamanho depois de remover: %d %d %d\n", activ->xsize, activ->ysize, activ->zsize);
